...
def detect_objects(image, objectClassifier):
    ...
    min_neighbors = 2
    haar_flags = 0
    objects = objectClassifier.detectMultiScale(image, haar_scale, min_neighbors, haar_flags, min_object_size) 
    return objects
...
def draw(photo):
    ...
    if binary_eye_image is not None:
        eye_circles = cv2.HoughCircles(binary_eye_image, cv2.HOUGH_GRADIENT, 3, 500, maxRadius = 40)
    ...
    if (cv2.waitKey(1) & 0xFF == ord('q')): #If we got a key press in less than 10ms
